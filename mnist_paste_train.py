...
path_name = '/home/holy/ML/Maix-TF-workspace/mnist/dataset_mynum/'
imagelist = os.listdir(path_name)
arr = np.random.randint(0,len(imagelist),size=300000)
...
for i in range(5000): #开始训练模型，循环训练5000次
    ...
    for kk in k_l:
        k000 = imagelist[kk]
        label = int(k000.split('_')[0])
        img = cv2.imread(path_name+k000, cv2.IMREAD_GRAYSCALE)
        img = img/255
        k00 = np.zeros((10),np.uint8)
        x_train.append(img)
        k00[label] = 1
        y_train.append(k00)
